...
def get_photos():
    ...
    try:
        ...
        photos = get_photo_service().get_photos(
            category=category,
            entity_id=entity_id,
            limit=min(limit, 100),  # Max 100 photos per request
            offset=offset
        )
        ...
        current_app.logger.error(f"Error fetching photos: {str(e)}")
...
def get_photo(photo_id):
    ...
    try:
        ...
        current_app.logger.error(f"Error fetching photo {photo_id}: {str(e)}")
...
def serve_thumbnail(photo_id):
    ...
    try:
        ...
        current_app.logger.error(f"Error serving thumbnail {photo_id}: {str(e)}")
...
def update_photo(photo_id):
    ...
    try:
        ...
        from src.models.user import db
        db.session.rollback()
        current_app.logger.error(f"Error updating photo {photo_id}: {str(e)}")
...
def set_primary_photo(photo_id):
    ...
    try:
        ...
        current_app.logger.error(f"Error setting primary photo {photo_id}: {str(e)}")
