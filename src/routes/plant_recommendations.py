...
def get_criteria_options():
    ...
    try:
        ...
        options = {
            ...
            'project_types': ['Garden', 'Landscape', 'Commercial', 'Residential', 'Public Space']
        }
...
def get_recommendation_history():
    ...
    try:
        ...
        requests = query.order_by(PlantRecommendationRequest.created_at.desc()) \
...
def export_recommendations():
    ...
    try:
        ...
        if not req or not req.recommended_plants:
            return jsonify({'error': 'Recommendation request not found or has no results'}), 404
        ...
        # Write header
        writer.writerow([
            'Plant ID', 'Plant Name', 'Common Name', 'Category',
            'Score', 'Height (min-max)', 'Width (min-max)',
            'Sun Requirements', 'Water Needs', 'Maintenance',
            'Native', 'Price', 'Match Reasons', 'Warnings'
        ])
