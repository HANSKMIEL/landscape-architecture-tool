...
def create_client():
    ...
    try:
        ...
        if existing_client:
            return (
                jsonify({"error": "Client with this email already exists"}),
                400,
            )
...
def get_client_stats():
    ...
    try:
        ...
        location_stats = (
            db.session.query(Client.city, db.func.count(Client.id))
            .filter(Client.city.isnot(None), Client.city != "")
            .group_by(Client.city)
            .order_by(db.desc(db.func.count(Client.id)))
            .limit(10)
            .all()
        )
