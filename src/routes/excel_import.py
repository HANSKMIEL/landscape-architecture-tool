...
def validate_import_file():
    ...
    try:
        ...
        try:
            ...
            return jsonify({"error": f"Fout bij lezen bestand: {str(e)}"}), 400
        ...
        return jsonify({"error": f"Fout bij valideren bestand: {str(e)}"}), 500
...
def validate_file_structure(df: pd.DataFrame, import_type: str) -> Dict[str, Any]:
    ...
    if import_type == 'plants' and len(df) > 0:
        if 'supplier_id' in df.columns:
            invalid_supplier_ids = df[df['supplier_id'].notna() & 
    ...
    if import_type == 'products' and len(df) > 0:
        if 'price' in df.columns:
            invalid_prices = df[df['price'].notna() & 
...
def get_import_recommendations(import_type: str, df: pd.DataFrame, missing_columns: List[str], extra_columns: List[str]) -> List[str]:
    ...
    if import_type == 'plants' and 'supplier_id' in df.columns:
        ...
        if missing_supplier_ids:
            recommendations.append(f"Leverancier IDs niet gevonden: {', '.join(map(str, missing_supplier_ids))}. Importeer eerst leveranciers.")
    ...
    if len(df) > 1000:
        recommendations.append("Groot bestand gedetecteerd. Overweeg het opsplitsen in kleinere bestanden voor betere prestaties.")
...
def process_import():
    ...
    try:
        ...
        if not validation_result['valid']:
            return jsonify({
                "error": "Bestand validatie gefaald",
        ...
        return jsonify({"error": f"Fout bij verwerken import: {str(e)}"}), 500
...
def download_template(import_type):
    ...
    try:
        templates = {
            ...
            'plants': {
                'columns': ['name', 'common_name', 'category', 'sun_requirements', 'water_needs', 'hardiness_zone', 'height_max', 'width_max', 'bloom_time', 'bloom_color', 'maintenance', 'supplier_id', 'notes', 'native_region', 'soil_type'],
                'sample_data': [
                    ['Acer palmatum', 'Japanse esdoorn', 'Boom', 'Halfschaduw', 'Gemiddeld', '6', '8.0', '6.0', 'April-Mei', 'Rood', 'Gemiddeld', '1', 'Prachtige herfstkleur', 'Japan', 'Humusrijk'],
                    ['Lavandula angustifolia', 'Lavendel', 'Vaste plant', 'Zon', 'Weinig', '5', '0.6', '0.8', 'Juli-September', 'Paars', 'Laag', '2', 'Geurig en bijenvriendelijk', 'Middellandse Zee', 'Doorlatend']
                ]
            },
            'products': {
                'columns': ['name', 'category', 'description', 'price', 'unit', 'supplier_id', 'stock_quantity', 'notes'],
                'sample_data': [
                    ['Potgrond universeel 40L', 'Grond en voeding', 'Hoogwaardige potgrond voor alle planten', '8.95', 'zak', '1', '50', 'Geschikt voor binnen en buiten'],
                    ['Tuinslang 25m', 'Gereedschap', 'Flexibele tuinslang met spuitpistool', '45.00', 'stuk', '2', '15', 'Inclusief koppelingen']
                ]
            },
            'clients': {
                'columns': ['name', 'email', 'phone', 'address', 'city', 'postal_code', 'country', 'client_type', 'company', 'notes'],
                'sample_data': [
                    ['Gemeente Amsterdam', 'projecten@amsterdam.nl', '+31 20 5551234', 'Stopera 1', 'Amsterdam', '1012 AB', 'Nederland', 'Zakelijk', 'Gemeente Amsterdam', 'Grote openbare projecten'],
                    ['Familie Jansen', 'j.jansen@email.nl', '+31 6 12345678', 'Dorpsstraat 25', 'Hilversum', '1234 AB', 'Nederland', 'Particulier', '', 'Privé tuin renovatie']
                ]
            }
        }
        ...
        return jsonify({"error": f"Fout bij genereren template: {str(e)}"}), 500
...
def get_import_status():
    ...
    try:
        ...
        return jsonify({"error": f"Fout bij ophalen import status: {str(e)}"}), 500