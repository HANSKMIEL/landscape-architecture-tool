...
from src.models.landscape import Project, Client, Plant, Product, Supplier, db
...
def generate_business_summary():
    ...
    try:
        ...
        ).filter(*date_filter).group_by(Project.status).all()
        ...
        ).join(Project).filter(*date_filter).group_by(
            Client.id, Client.name, Client.company
        ).order_by(db.desc('project_count')).limit(10).all()
        ...
        # Most used plants
        plant_usage = db.session.query(
            Plant.name,
            Plant.scientific_name,
            db.func.count(db.distinct(Project.id)).label('project_count')
        ).join(Project.plants).join(Project).filter(*date_filter).group_by(
            Plant.id, Plant.name, Plant.scientific_name
        ).order_by(db.desc('project_count')).limit(10).all()
        ...
        product_usage = db.session.query(
            ...
            Product.category,
            db.func.count(db.distinct(Project.id)).label('project_count')
        ).join(Project.products).join(Project).filter(*date_filter).group_by(
...
def generate_business_summary_pdf(data):
    ...
    try:
        ...
        # Generation info
        story.append(Paragraph(f"Generated: {datetime.utcnow().strftime('%B %d, %Y at %H:%M')}", styles['Normal']))
        ...
        if budget_stats['total_budget'] > 0:
            ...
            budget_data = [
                ...
                ['Utilization Rate', f"{budget_stats['utilization_rate']:.1f}%"]
            ]
        ...
        if data['top_clients']:
            ...
            client_data = [['Client', 'Company', 'Projects', 'Total Budget (€)']]
            ...
            client_table = Table(client_data, colWidths=[1.5*inch, 1.5*inch, 1*inch, 1.5*inch])
...
def generate_project_report_pdf(data):
    ...
    try:
        ...
        project_data = [
            ...
            ['Area Size', f"{project['area_size']} m²" if project['area_size'] else '-'],
            ['Budget', f"€{project['budget']:,.2f}" if project['budget'] else '-'],
            ['Spent', f"€{project['spent']:,.2f}" if project['spent'] else '-'],
            ['Start Date', project['start_date'][:10] if project['start_date'] else '-'],
            ['End Date', project['end_date'][:10] if project['end_date'] else '-']
        ]
        ...
        if data['plants']:
            ...
            plant_data = [['Plant Name', 'Scientific Name', 'Category', 'Sun', 'Water']]
            ...
            plant_table = Table(plant_data, colWidths=[1.5*inch, 1.5*inch, 1*inch, 0.8*inch, 0.8*inch])
            plant_table.setStyle(TableStyle([
                ('BACKGROUND', (0, 0), (-1, 0), colors.HexColor('#2D5016')),
                ('TEXTCOLOR', (0, 0), (-1, 0), colors.whitesmoke),
                ('ALIGN', (0, 0), (-1, -1), 'LEFT'),
        ...
        if data['products']:
            ...
            product_table = Table(product_data, colWidths=[2*inch, 1.2*inch, 1*inch, 1.3*inch])
...
def generate_plant_usage_report():
    ...
    try:
        ...
        plant_usage = db.session.query(
            ...
            Plant.category,
            db.func.count(db.distinct(Project.id)).label('project_count')
        ).join(Project.plants).join(Project).group_by(
            Plant.id, Plant.name, Plant.scientific_name, Plant.category
        ).order_by(db.desc('project_count')).all()
        ...
        category_stats = db.session.query(
            Plant.category,
            db.func.count(db.distinct(Project.id)).label('project_count')
        ).join(Project.plants).join(Project).filter(
            Plant.category.isnot(None)
        ).group_by(Plant.category).order_by(db.desc('project_count')).all()
...
def generate_supplier_performance_report():
    ...
    try:
        ...
        supplier_stats = db.session.query(
            ...
            db.func.count(db.distinct(Project.id)).label('project_count')
        ).outerjoin(Product).outerjoin(Project.products).outerjoin(Project).group_by(
            Supplier.id, Supplier.name, Supplier.contact_person, Supplier.email
        ).order_by(db.desc('product_count')).all()
        
        supplier_data = []
        for name, contact, email, product_count, project_count in supplier_stats:
