...
from src.models.landscape import Project, Client, Plant, Product, db, project_plants, project_products
...
def get_projects():
    ...
    try:
        ...
        for project in projects.items:
            ...
            plants = db.session.query(Plant).join(project_plants).filter(
                project_plants.c.project_id == project.id
            ).all()
            
            products = db.session.query(Product).join(project_products).filter(
                project_products.c.project_id == project.id
            ).all()
            ...
            projects_data.append({
                ...
                'plants': [{'id': p.id, 'name': p.name, 'scientific_name': p.scientific_name} for p in plants],
...
def add_product_to_project(project_id):
    ...
    try:
        ...
        if product in project.products:
            return jsonify({'error': 'Product is already in this project'}), 400
...
@projects_bp.route('/api/projects/<int:project_id>/products/<int:product_id>', methods=['DELETE'])
