...
DEFAULT_SETTINGS = {
    "appearance": {
        ...
        "font_size": "medium",
        "branding": {"company_name": "Landscape Architecture Tool", "logo_url": "", "custom_css": ""},
    },
    "data_management": {
        "auto_backup": True,
        "backup_frequency": "weekly",
        "excel_import": {"default_supplier": "", "auto_validate": True, "duplicate_handling": "skip"},
        "ai_assistant": {"enabled": True, "auto_suggestions": True, "confidence_threshold": 0.8},
    },
    "integrations": {
        "apis": {
            "vectorworks": {"enabled": False, "api_key": "", "endpoint": ""},
            "crm": {"enabled": False, "type": "", "credentials": {}},
        },
        "n8n": {
            "enabled": True,
            "webhook_url": "",
            "workflows": {"client_onboarding": True, "project_updates": True, "inventory_alerts": True},
        },
    },
    "reports": {
        "default_format": "pdf",
        ...
        "templates": {"company_header": True, "watermark": False, "custom_footer": ""},
    },
    "security": {
        ...
        "two_factor": {"enabled": False, "method": "email"},
    },
}
    ...
    try:
        ...
        if "security" in client_settings:
            ...
            client_settings["security"] = {
                ...
                "two_factor": {"enabled": client_settings["security"].get("two_factor", {}).get("enabled", False)},
            }

        # Remove API keys and credentials
        if "integrations" in client_settings:
            for _integration_type, config in client_settings["integrations"].items():
                if isinstance(config, dict):
                    for _service, service_config in config.items():
        ...
        return jsonify({"settings": client_settings, "status": "success"}), 200
...
def reset_settings():
    ...
    try:
        if save_settings(DEFAULT_SETTINGS.copy()):
            return jsonify({"message": "Settings reset to defaults", "status": "success"}), 200
...
def export_settings():
    ...
    try:
        ...
        if "integrations" in export_settings:
            for _integration_type, config in export_settings["integrations"].items():
                if isinstance(config, dict):
                    for _service, service_config in config.items():
                        if isinstance(service_config, dict):
                            # Remove sensitive keys
        ...
        return jsonify({"settings": export_settings, "exported_at": datetime.now().isoformat(), "version": "1.0"}), 200
...
def import_settings():
    ...
    try:
        ...
        if save_settings(merged_settings):
            return jsonify({"message": "Settings imported successfully", "status": "success"}), 200
