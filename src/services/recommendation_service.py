...
from src.services.plant_recommendation import PlantRecommendationEngine, RecommendationCriteria
...
class RecommendationService:
    ...
    def _convert_criteria(self, criteria: Dict) -> RecommendationCriteria:
        ...
        direct_fields = [
            'hardiness_zone', 'soil_type', 'soil_ph', 'maintenance_level',
            'budget_range', 'native_preference', 'wildlife_friendly',
            'deer_resistant_required', 'pollinator_friendly_required',
            'container_planting', 'screening_purpose', 'hedging_purpose',
            'groundcover_purpose', 'slope_planting'
        ]
    ...
    def _create_criteria_match(self, plant_score, criteria: Dict) -> Dict:
        ...
        if 'sun_requirements' in criteria:
            match['sun_requirements'] = self._check_sun_match(
                plant_score.plant.sun_requirements, 
                criteria['sun_requirements']
            )
        
        if 'soil_type' in criteria:
            match['soil_type'] = self._check_soil_match(
                plant_score.plant.soil_type,
                criteria['soil_type']
            )
        ...
        if 'hardiness_zone' in criteria:
            match['hardiness_zone'] = self._check_hardiness_match(
                plant_score.plant.hardiness_zone,
                criteria['hardiness_zone']
            )
