...
class PlantRecommendationEngine:
    ...
    def _score_environmental(
        ...
        if criteria.hardiness_zone and plant.hardiness_zone:
            ...
            if criteria.hardiness_zone in plant.hardiness_zone:
                ...
                warnings.append(f"Hardiness zone mismatch: plant {plant.hardiness_zone}, site {criteria.hardiness_zone}")
        ...
        if criteria.sun_exposure and plant.sun_requirements:
            ...
            if self._match_sun_exposure(criteria.sun_exposure, plant.sun_requirements):
                ...
                warnings.append(f"Sun exposure concern: plant needs {plant.sun_requirements}, site has {criteria.sun_exposure}")
        ...
        if criteria.soil_ph and plant.soil_ph_min and plant.soil_ph_max:
            ...
            if plant.soil_ph_min <= criteria.soil_ph <= plant.soil_ph_max:
                ...
                score += 0.2
                warnings.append(f"pH concern: plant prefers {plant.soil_ph_min}-{plant.soil_ph_max}, site is {criteria.soil_ph}")
        ...
        if criteria.moisture_level and plant.water_needs:
            ...
            if self._match_moisture_level(criteria.moisture_level, plant.water_needs):
                ...
                score += 0.4
                warnings.append(f"Water needs mismatch: plant needs {plant.water_needs}, site has {criteria.moisture_level}")
    ...
    def _score_design(
        ...
        if (criteria.desired_height_min is not None or criteria.desired_height_max is not None) and \
           (plant.height_min is not None or plant.height_max is not None):
            ...
            if self._size_compatible(criteria.desired_height_min, criteria.desired_height_max, 
                ...
                score += 0.3
                warnings.append(f"Height difference: plant {plant.height_min}-{plant.height_max}m, desired {criteria.desired_height_min}-{criteria.desired_height_max}m")
        ...
        if (criteria.desired_width_min is not None or criteria.desired_width_max is not None) and \
           (plant.width_min is not None or plant.width_max is not None):
            ...
            if self._size_compatible(criteria.desired_width_min, criteria.desired_width_max,
    ...
    def _score_maintenance(
        ...
        if criteria.maintenance_level and plant.maintenance:
            ...
            if self._match_maintenance_level(criteria.maintenance_level, plant.maintenance):
                ...
                score += 0.4
                warnings.append(f"Maintenance mismatch: plant needs {plant.maintenance}, preference is {criteria.maintenance_level}")
        ...
        if criteria.budget_range and plant.price:
            ...
            if budget_compatible:
                ...
                score += 0.2
                warnings.append(f"Price concern: ${plant.price} may exceed {criteria.budget_range} budget")
        
        # Pest and disease resistance
        if plant.pest_resistance or plant.disease_resistance:
            total_factors += 1
            resistance_score = 0
            if plant.pest_resistance and plant.pest_resistance.lower() in ['high', 'medium']:
                resistance_score += 0.5
                match_reasons.append(f"Good pest resistance ({plant.pest_resistance})")
            if plant.disease_resistance and plant.disease_resistance.lower() in ['high', 'medium']:
                resistance_score += 0.5
    ...
    def _score_special(
        ...
        if criteria.wildlife_friendly:
            total_factors += 1
            if plant.wildlife_value and plant.wildlife_value.lower() in ['high', 'medium']:
                score += 1.0
    ...
    def _size_compatible(self, criteria_min: Optional[float], criteria_max: Optional[float],
                        plant_min: Optional[float], plant_max: Optional[float]) -> bool:
