...
class RecommendationAlgorithm:
    ...
    def calculate_score(self, plant_data: Dict, criteria: Dict) -> float:
        ...
        for criterion, weight in self.criteria_weights.items():
            if criterion in criteria:
                ...
                elif criterion in ['sun_requirements', 'soil_type', 'plant_type', 'water_requirements']:
    ...
    def _is_sun_requirement_field(self, plant_value: str, criteria_value: str) -> bool:
        ...
        return (plant_value.lower().replace(' ', '_') in sun_values or 
                criteria_value.lower().replace(' ', '_') in sun_values)
    ...
    def _is_soil_field(self, plant_value: str, criteria_value: str) -> bool:
        ...
        return any(soil in plant_value.lower() or soil in criteria_value.lower() 
    ...
    def _score_sun_compatibility(self, plant_sun: str, criteria_sun: str) -> float:
        ...
        sun_compatibility = {
            'full_sun': {'full_sun': 1.0, 'partial_sun': 0.7, 'partial_shade': 0.3, 'full_shade': 0.0},
            'partial_sun': {'full_sun': 0.7, 'partial_sun': 1.0, 'partial_shade': 0.8, 'full_shade': 0.3},
            'partial_shade': {'full_sun': 0.3, 'partial_sun': 0.8, 'partial_shade': 1.0, 'full_shade': 0.7},
            'full_shade': {'full_sun': 0.0, 'partial_sun': 0.3, 'partial_shade': 0.7, 'full_shade': 1.0},
        }
