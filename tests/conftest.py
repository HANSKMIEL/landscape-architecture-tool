...
def app():
    ...
    if database_url and "postgresql" in database_url:
        ...
        if "?" in database_url:
            database_url += "&pool_size=5&max_overflow=10&pool_timeout=30&pool_recycle=300"
        else:
            database_url += "?pool_size=5&max_overflow=10&pool_timeout=30&pool_recycle=300"
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt == max_retries - 1:
                    ...
                    import warnings
                    warnings.warn(f"Database setup failed after {max_retries} attempts: {e}")
                else:
                    # Wait before retry
                    import time
                    time.sleep(1)
            ...
            if 'postgresql' in engine_name:
                ...
                for model in [
                    ...
                    try:
                        ...
                        if time.time() - start_time > 5:  # 5 second timeout per operation
                            break
                    except Exception:
                        pass
