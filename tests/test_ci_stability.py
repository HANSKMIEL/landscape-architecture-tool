...
class TestCIStability:
    ...
    def test_database_cleanup_performance(self, app):
        ...
        with app.app_context():
            ...
            # Measure cleanup time (transaction-based - should be instant)
            start_time = time.time()
            # In transaction-based isolation, cleanup is handled by rollback
            db.session.rollback()
    ...
    @pytest.mark.skip(
        reason="SQLite doesn't support true concurrent access - test is not reliable in CI environment"
    )
    ...
    def test_timeout_resilience(self, app):
        ...
        with app.app_context():
            ...
            assert (
                total_time < 5
            ), f"Database operations took {total_time:.2f}s (too slow)"
        ...
        with app.app_context():
            ...
            try:
                ...
                with patch.object(
                    db_session,
                    "commit",
                    side_effect=Exception("Simulated commit error"),
                ):
    ...
    def test_overall_test_suite_timing(self, app):
