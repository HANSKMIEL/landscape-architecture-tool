...
class TestDashboardService(DatabaseTestMixin):
    ...
    def test_get_dashboard_summary_with_data(self, app_context, client_factory, project_factory, plant_factory, supplier_factory):
    ...
    def test_get_project_analytics_with_data(self, app_context, client_factory, project_factory):
        ...
        now = datetime.utcnow()
        projects = [
            project_factory(client=client1, status="active", budget=5000.0, created_at=now - timedelta(days=5)),
            project_factory(client=client1, status="completed", budget=8000.0, created_at=now - timedelta(days=10)),
            project_factory(client=client2, status="active", budget=6000.0, created_at=now - timedelta(days=15)),
            project_factory(client=client1, status="planning", budget=4000.0, created_at=now - timedelta(days=20)),
        ]
    ...
    def test_get_plant_analytics_with_data(self, app_context, plant_factory, project_factory, client_factory):
        ...
        plant4 = plant_factory(name="Maple Tree", category="Tree", sun_exposure="partial_shade", native=False)
...
class TestDashboardServiceIntegration(DatabaseTestMixin):
    ...
    def test_comprehensive_dashboard_scenario(self, app_context, client_factory, project_factory, plant_factory, supplier_factory, product_factory):
