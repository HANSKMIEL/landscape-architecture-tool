...
class ComprehensiveUITester:
    ...
    def analyze_language_change(self, before_text, after_text, target_lang, target_name):
        ...
        if len(found_expected) > 0:
            self.log_test(
                f"Language Switch to {target_name}",
                "PASS",
                f"Found {len(found_expected)} expected terms: {found_expected[:3]}...",
            )
        else:
            self.log_issue("Language Translation", f"No expected {target_name} terms found after switching")
    ...
    def test_individual_panel(self, panel_name, selectors):
        ...
        try:
            ...
            for selector in selectors:
                try:
                    panel_link = WebDriverWait(self.driver, 3).until(EC.element_to_be_clickable((By.XPATH, selector)))
                    break
    ...
    def test_settings_panel_functionality(self):
        ...
        try:
            ...
            try:
                self.driver.find_element(
                    By.XPATH, "//select[option[contains(text(), 'English') or contains(text(), 'Nederlands')]]"
                )
    ...
    def test_plants_input_fields(self):
        ...
        try:
            ...
            add_buttons = self.driver.find_elements(
                By.XPATH, "//button[contains(text(), 'Add') or contains(text(), 'Toevoegen')]"
            )
            ...
            try:
                cancel_button = self.driver.find_element(
                    By.XPATH, "//button[contains(text(), 'Cancel') or contains(text(), 'Annuleren')]"
                )
                cancel_button.click()
            except Exception:
