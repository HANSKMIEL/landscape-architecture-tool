...
class VPSEnhancedTester:
    ...
    def log_issue(self, category, issue):
        ...
        self.issues_found.append(
            {"category": category, "issue": issue, "timestamp": time.strftime("%Y-%m-%d %H:%M:%S")}
        )
        ...
        try:
            ...
            try:
                ...
                login_button = self.driver.find_element(
                    By.XPATH, "//button[contains(., 'Log') or contains(., 'Inlog')]"
                )
                login_button.click()
                ...
                WebDriverWait(self.driver, 10).until(
                    EC.presence_of_element_located(
                        (By.XPATH, "//h1[contains(., 'Dashboard') or contains(., 'Landschap')]")
                    )
                )
    ...
    def compare_language_texts(self, dutch_texts, english_texts):
        ...
        if len(english_only) > 0:
            ...
            self.log_issue(
                "Language Translation", "No English translations detected - language switching may not be working"
            )
        ...
        if len(unchanged) > 0:
            self.log_issue(
                "Language Translation", f"These texts appear in both languages (may be untranslated): {list(unchanged)}"
            )
    ...
    def test_all_panels_comprehensive(self):
        ...
        panels_to_test = [
            (
                "Settings",
                [
                    "//a[contains(., 'Settings') or contains(., 'Instellingen')]",
                    "//nav//a[contains(@href, 'settings')]",
                ],
            ),
            (
                "Suppliers",
                [
                    "//a[contains(., 'Suppliers') or contains(., 'Leveranciers')]",
                    "//nav//a[contains(@href, 'suppliers')]",
                ],
            ),
            ("Plants", ["//a[contains(., 'Plants') or contains(., 'Planten')]", "//nav//a[contains(@href, 'plants')]"]),
            (
                "Products",
                ["//a[contains(., 'Products') or contains(., 'Producten')]", "//nav//a[contains(@href, 'products')]"],
            ),
            (
                "Clients",
                ["//a[contains(., 'Clients') or contains(., 'Klanten')]", "//nav//a[contains(@href, 'clients')]"],
            ),
            (
                "Projects",
                ["//a[contains(., 'Projects') or contains(., 'Projecten')]", "//nav//a[contains(@href, 'projects')]"],
            ),
            (
                "Reports",
                ["//a[contains(., 'Reports') or contains(., 'Rapporten')]", "//nav//a[contains(@href, 'reports')]"],
            ),
        ...
        self.log_test(
            "Panel Testing Summary",
            "PASS" if working_panels == total_panels else "WARN",
    ...
    def test_settings_panel(self):
        ...
        try:
            ...
            settings_sections = self.driver.find_elements(
                By.XPATH, "//div[contains(@class, 'tab') or contains(@class, 'section')]"
            )
            ...
            language_settings = self.driver.find_elements(
                By.XPATH, "//*[contains(., 'Language') or contains(., 'Taal')]"
            )
    ...
    def test_plants_panel_input_fields(self):
        ...
        try:
            ...
            add_buttons = self.driver.find_elements(
                By.XPATH, "//button[contains(., 'Add') or contains(., 'Toevoegen')]"
            )
        ...
        try:
            ...
            for i, char in enumerate(test_text):
                ...
                if current_value != expected_value:
                    issues_found.append(
                        f"Value mismatch after '{char}': expected '{expected_value}', got '{current_value}'"
                    )
    ...
    def test_generic_panel_functionality(self, panel_name):
        ...
        try:
            ...
            loading_elements = self.driver.find_elements(
                By.XPATH, "//*[contains(., 'Loading') or contains(., 'Laden')]"
            )
            if loading_elements:
