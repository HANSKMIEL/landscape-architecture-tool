...
class AdminTestAndFix:
    ...
    def test_authentication_system(self):
        ...
        try:
            ...
            self.issues_found.append({
                "type": "auth_error",
                "description": f"Authentication system error: {e}"
            })
    ...
    def fix_ui_text_issues(self):
        ...
        if not ui_report_files:
            ...
            subprocess.run([
                "python", 
                str(self.repo_path / "scripts" / "ui_text_analysis.py")
            ], cwd=self.repo_path)
            ui_report_files = list(reports_dir.glob("ui_text_analysis_*.json"))
        ...
        if ui_report_files:
            ...
            if translation_issues:
                ...
                disabled_translation_files = [
                    issue["file"] for issue in translation_issues 
    ...
    def generate_issue_report(self):
        ...
        report_file = self.repo_path / "reports" / f"admin_comprehensive_test_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
