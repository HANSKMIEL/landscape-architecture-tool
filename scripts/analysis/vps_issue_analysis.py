...
class VPSIssueAnalyzer:
    ...
    def identify_missing_translation_patterns(self, nl_data, en_data):
        ...
        if untranslated_nl:
            self.log_issue("Translation Quality", f"Found {len(untranslated_nl)} potentially untranslated Dutch values", "LOW")
        
        if untranslated_en:
            self.log_issue("Translation Quality", f"Found {len(untranslated_en)} potentially untranslated English values", "LOW")
    ...
    def analyze_settings_component(self):
        ...
        try:
            ...
            settings_sections = [
                "appearance", "api", "ai", "bulk", "reports", "security"
            ]
