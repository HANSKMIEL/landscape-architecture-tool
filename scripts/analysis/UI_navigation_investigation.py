    ...
    try:
        ...
        try:
            ...
            login_buttons = driver.find_elements(
                By.XPATH,
                "//button[contains(text(), 'Login') or contains(text(), 'Sign In') or contains(text(), 'Inloggen')]",
            )
            if login_buttons:
                results["findings"]["login_button"] = f"Found {len(login_buttons)} login button(s)"

            # Check for demo credentials display
            demo_text = driver.find_elements(
                By.XPATH, "//*[contains(text(), 'admin') or contains(text(), 'Demo') or contains(text(), 'Test')]"
            )
            if demo_text:
        ...
        try:
            ...
            results["navigation_issues"].append(f"Login form elements not found: {e!s}")
        ...
        try:
            ...
            results["navigation_issues"].append(f"Dashboard check failed: {e!s}")
        ...
        try:
            ...
            results["navigation_issues"].append(f"Plants navigation failed: {e!s}")
        ...
        try:
            ...
            results["navigation_issues"].append(f"Add Plant button search failed: {e!s}")
        ...
        try:
            ...
            if name_input:
                ...
                for i, char in enumerate(test_text):
                    try:
                        ...
                        results["navigation_issues"].append(f"Input field test failed at character {i}: {e!s}")
                        break
            ...
            results["navigation_issues"].append(f"Input field testing failed: {e!s}")
        ...
        try:
            ...
            structural_elements = {
                ...
                "modals": len(
                    driver.find_elements(By.XPATH, "//div[contains(@class, 'modal') or contains(@class, 'fixed')]")
                ),
            }
            ...
            results["navigation_issues"].append(f"Page structure analysis failed: {e!s}")
