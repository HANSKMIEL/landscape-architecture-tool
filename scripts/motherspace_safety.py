...
class MotherSpaceSafetyManager:
        ...
        if lock_file.exists():
            try:
                ...
                if lock_age > self.config["concurrency_timeout"]:
                    ...
                    print(f"⏳ Operation {operation_name} is locked by {lock_data.get('actor')} (remaining: {self.config['concurrency_timeout'] - lock_age:.0f}s)")
                    return False
    ...
    def check_actor_cooldown(self, actor: str, operation: str) -> bool:
        ...
        if cooldown_file.exists():
            try:
                ...
                cooldown_remaining = self.config["actor_cooldown_period"] - (datetime.now() - last_operation).total_seconds()
            ...
            try:
                ...
                if rate_hour == current_hour:
                    ...
                    if count >= self.config["max_operations_per_hour"]:
                        print(f"🚫 Rate limit exceeded for {operation} ({count}/{self.config['max_operations_per_hour']} operations this hour)")
                        return False
        ...
        if pattern_file.exists():
            try:
                ...
                if len(similar_operations) >= 3:
                    print(f"🤖⚠️ Bot loop detected: {actor} has performed similar operation '{issue_title}' {len(similar_operations)} times in the last hour")
                    return False
    ...
    def is_safe_operation(self, actor: str, operation: str, issue_data: dict[str, Any]) -> dict[str, Any]:
        ...
        if existing_issue:
            ...
            print(f"🔍 Found existing tracking issue #{existing_issue['issue_number']} for fingerprint {fingerprint[:8]}...")
